...
int main (void)
{
    ...
    prussdrv_exec_program (PRU_NUM, "./Rover5_PRU0.bin");
    ...
    while(counter != 0){
    	...
    	duration = (duration*4*5*340.29*100)/(1000000000*2);	//calculate distance from duration
    	ping_counter = sharedMem_int[OFFSET_SHAREDRAM+4];		//get ping count
    	printf("\tDistance = %d cm, Ping counter = %d \r\n", duration, ping_counter);
